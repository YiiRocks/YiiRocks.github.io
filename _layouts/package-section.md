...
<div class="doc-page">
    <div class="doc-header d-flex align-items-center gap-4">
        <a href="{{ pkg.docsUrl }}" class="doc-header__icon d-flex align-items-center justify-content-center flex-shrink-0" style="background:{{ pkg.tint }};"><img src="{{ pkg.logo | relative_url }}" alt="{{ pkg.name }}" class="pkg-icon-img"></a>
        <div>
            <a href="{{ pkg.docsUrl }}" class="link-body-emphasis link-underline-opacity-0"><h1 class="doc-header__name">{{ pkg.name }}</h1></a>
            <div class="doc-header__tagline fw-semibold">{{ pkg.tagline }}</div>
        </div>
    </div>
...
{% include doc_sidebar.md pkgId=page.pkgId section=page.section %}
        <div class="doc-section-content">
            ...
            <div class="doc-section-prevnext d-flex justify-content-between gap-4">
{% if current_index > 0 %}
...
{% assign next = pkg.sections[next_index] %}
                <a href="{{ pkg.docsUrl }}{{ next.slug }}/" class="doc-section-prevnext__next">{{ next.title }} &rarr;</a>
{% endif %}
            </div>
        </div>
    </div>
