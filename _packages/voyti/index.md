...
features: 
  ...
  - label: "REST API: user management"
    detail: "JSON user CRUD (Bearer-token auth), with optional per-user request throttling (429 responses, X-Rate-Limit-* headers)"
  - label: "SCIM provisioning"
    detail: "SCIM 2.0 user and RBAC-group provisioning for identity providers, with discovery, search, bulk operations, and ETag-based concurrency control"
  - label: "REST API: client login &amp; self-service"
    detail: "Credential login, account/session self-service, admin RBAC/audit-log, and dynamic 2FA/social-login/GDPR bridges over JSON, for a browser-based single-page application (SPA) or any other stateless client"
